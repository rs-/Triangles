...
Module StreamTerminal (Import Ax : StreamAxioms).
  ...
  Next Obligation.
    ...
    apply bisim_intro with (λ s₁ s₂ ∙ ∃ x, s₁ = cosubst f x ∧ s₂ = cosubst g x); [intros..|eauto].
    - destruct H as (x' & -> & ->).
      unfold cosubst; repeat rewrite head_corec.
      equivify. apply eq_fg.
    - destruct H as (x' & -> & ->).
      exists (tail x').
      split.
      + unfold cosubst at 1. rewrite tail_corec. reflexivity.
  ...
  Next Obligation.
    simpl. intros.
    apply bisim_intro with (λ s₁ s₂ ∙ ∃ x, s₁ = cosubst head x ∧ s₂ = x); [clean_hyps; intros..|eauto].
    - destruct H as (x & -> & ->).
  ...
  (** counit-cobind **)
  Next Obligation.
    repeat intro. simpl. unfold cosubst. now rewrite head_corec.
  Qed.
  (** cobind-cobind **)
  Next Obligation.
  ...
  (** ** Stream coalgebra **)
  Program Definition 𝑻𝒂𝒊𝒍 : Comodule.Morphism ([𝑺𝒕𝒓]) ([𝑺𝒕𝒓]) :=
    Comodule.make ⦃ α ≔ λ A ∙ Π.make 𝒕𝒂𝒊𝒍 ⦄.
  (** tail-cong **)
  Next Obligation.
    intros A x y eq_xy; now cong.
  Qed.
  (** α-commutes **)
  Next Obligation.
    intros C D f x; simpl. unfold cosubst. now rewrite tail_corec.
  Qed.
  ...
  Section Defs.
    ...
    Proof.
      ...
      [clean_hyps; intros..|eauto].
      - destruct H as (x & y & eq_xy & -> & ->).
        unfold tau. repeat rewrite head_corec.
        equivify. now cong.
      - destruct H as (x & y & eq_xy & -> & ->).
    ...
    Proof.
      intros A t t' eq_tt'. etrans. cong. apply eq_tt'.
      simpl. unfold tau. now rewrite head_corec.
    Qed.

    Lemma tau_cobind : ∀ A B (f : STR A ⇒ 𝑬𝑸 B) x, Tau (T⋅cobind (f ∘ Tau) x) ∼ STR⋅cobind f (Tau x).
    Proof.
      intros A B f x.
      ...
      - destruct H as (x & ? & ?).
        ...
        symmetry. equivify. etrans. apply (counit_cobind T). refl.
      - destruct H as (x & ? & ?).
        exists (T⋅tail x). split.
        etrans. eapply tail_cong. apply H.
        simpl. unfold tau. repeat rewrite tail_corec.
        apply tau_cong. etrans. apply (α_commutes (Stream.tail Tr)). refl.
        etrans. eapply tail_cong. apply H0.
  ...
  (** τ-commutes **)
  Next Obligation.
    repeat intro. simpl. unfold tau. repeat rewrite tail_corec. reflexivity.
  Qed.
  ...
  (** top-unique **)
  Next Obligation.
    intros A f X x. simpl.
    apply bisim_intro with (λ s₁ s₂ ∙ ∃ x, s₁ ∼ ⟨f⟩ _ x ∧ s₂ ∼ tau A x); [clean_hyps; intros..|].
    - destruct H as (? & ? & ?).
      etransitivity. eapply head_cong. apply H.
      symmetry; etransitivity. eapply head_cong. apply H0.
      unfold tau. rewrite head_corec.
      symmetry. etransitivity. symmetry. eapply xxx. eapply (τ_counit (Stream.τ f)).
      reflexivity.
    - destruct H as (? & ? & ?).
      exists (Stream.tail A _ x). split.
      + eapply bisim_trans. eapply tail_cong. apply H.
        eapply bisim_trans. eapply symmetry. eapply (Stream.τ_commutes f). reflexivity.
      + eapply transitivity. eapply tail_cong. apply H0.
